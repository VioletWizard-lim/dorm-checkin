...
- 학생 카드 리스트: 아바타, 이름, "학번 · 반", 상태 배지(재실/외출중/무단외출/자리비움/명령퇴사), 상태별 조작:
  - **재실** → "외출 체크" 버튼(아래 설명) + "상태 표시..." 드롭다운(무단외출/자리비움으로 직접 표시, 순회 중 발견 시 사용)
  - **외출중** → "복귀 체크" 버튼(재실로)
  - **무단외출/자리비움** → "재실로 되돌리기" 버튼만 노출(자동 판단 없이 전부 수동 — 순회하는 교사가 직접 표시·해제)
  - **명령퇴사**(기간제 상태, `students/{grade}/{studentId}.leaveOfAbsence`가 조회 중인 날짜를 포함할 때) → 조작 버튼 없이 "학생 명단 관리에서 설정"만 표시(설정은 students.html에서 함). 이 기간 동안은 무단외출·자리비움 판정에서 제외되고 다른 어떤 상태보다 우선 표시됨
...
- 패널 5개(가로 배치, 화면이 좁으면 줄바꿈): **무단외출** > **외출중**(빨강 계열) > **자리비움** > **명령퇴사** > **오늘 방과후**(파랑 계열) — 명령퇴사 중인 학생은 그 기간 동안 이 패널에만 나타나고 무단외출·외출중·자리비움·방과후 패널에서는 제외됨(check.html/students.html 참고)
- 각 항목: 아바타, 이름, "학번 · 반", 외출/무단외출/자리비움 시각 또는 방과후 활동명 또는 명령퇴사 종료일
...
- 좌석 카드 색상 우선순위: **명령퇴사(보라) > 무단외출(주황) > 외출중(빨강) > 자리비움(황금) > 오늘 방과후(파랑) > 재실(회색) > 빈자리(점선)** — 무단외출/자리비움/명령퇴사의 의미는 check.html/students.html 참고. 좌석 배치판은 항상 오늘 기준만 표시함
- 학년관리자는 자기 `managedRooms`에 속한 실만 편집 가능(권한 규칙 참고)

### 5. 학생 명단 관리 (`students.html`)
- admin·gradeManager는 항상 접근 가능. teacher는 담임(담당 반, `managedClasses`)이 배정되어 있을 때만 접근 가능 — 아무 반도 배정 안 된 일반 teacher는 접근 시 `check.html`로 리다이렉트
- 학년 탭: admin은 1/2/3학년 전체, gradeManager는 자기 `managedGrades`에 속한 학년 전체(반 구분 없음), teacher는 자기 `managedClasses`에 반이 있는 학년만(그 학년 탭 안에서도 담당 반 학생만 보임)
...
- 학생별 **명령퇴사 기간**(선택) 설정: "+ 학생 추가"/"수정" 폼에 시작일·종료일·사유 입력란이 있음. 시작일·종료일은 하나만 입력하면 alert로 막고(둘 다 입력하거나 둘 다 비워야 함), 종료일이 시작일보다 빠르면 저장을 막음. 저장하면 `students/{grade}/{studentId}.leaveOfAbsence`에 반영되고, 그 기간 동안 check.html·display.html·seat.html에서 "명령퇴사"로 표시되며 무단외출·자리비움 판정에서 제외됨(둘 다 비우고 저장하면 해제)
- `check.html` 상단에 이 화면으로 가는 "학생 명단 관리" 링크가 admin·gradeManager·(담당 반이 있는)teacher에게만 노출됨

### 6. 계정 관리 (`accounts.html`)
- admin 전용(admin이 아니면 `check.html`로 리다이렉트). `check.html` 상단에 이 화면으로 가는 "계정 관리" 링크가 admin에게만 노출됨
- 상단: 등록된 계정 목록 — 아이디·이름·역할 배지(teacher/gradeManager/admin/studyHallSupervisor), gradeManager는 담당 학년·담당 실을, teacher는 담당 반(있는 경우만)을 함께 표시
  - 본인 계정 행에는 "정보 수정" 버튼이 없음(관리자가 실수로 자기 자신을 강등해 잠기는 것을 방지)
  - "정보 수정" 클릭 시 그 계정 행이 인라인 편집 폼으로 바뀜: 이름 입력란, 역할 토글(teacher/gradeManager/admin/studyHallSupervisor) → 저장 시 `users/{uid}` set. 이름은 매년 같은 아이디를 다른 담당자가 이어받는 경우(예: "1학년부장" 계정)를 대비해 언제든 바꿀 수 있게 함
...
| 기능 | teacher | gradeManager | admin | studyHallSupervisor |
|---|---|---|---|---|
| 외출 체크 입력/조회 (+ 무단외출·자리비움 수동 표시, 지난 기록 수정) | ✅ | ✅ | ✅ | ✅ (이것만 가능) |
| 현황판·좌석배치판 열람 | ✅ | ✅ | ✅ | ❌ |
| 담당 실의 좌석 배치 편집 | ❌ | ✅ (담당 실만) | ✅ (전체) | ❌ |
| 학생 명단·방과후 요일·명령퇴사 기간 등록/수정 | ✅ (담당 반만, `managedClasses` 배정된 경우) | ✅ (담당 학년 전체) | ✅ (전체) | ❌ |
| 실 추가/삭제·이름 변경·대상 학년 지정 | ❌ | ❌ | ✅ | ❌ |
| 교사 계정 추가/역할 지정 | ❌ | ❌ | ✅ | ❌ |

studyHallSupervisor는 외출 체크 화면(`check.html`) 외에는 아무 화면도 접근할 수 없음(다른 화면 URL로 직접 이동해도 check.html로 리다이렉트).
...
users/
  {uid}: {
    ...
    role: "teacher" | "gradeManager" | "admin" | "studyHallSupervisor",
...
students/
  {grade}/                                // "1" | "2" | "3"
    {studentId}: {
      ...
      leaveOfAbsence?: { from: string, to: string, reason?: string }  // "명령퇴사" 기간(YYYY-MM-DD, 둘 다 있어야 유효). students.html에서 설정. 오늘이 이 구간에 포함되면 check.html/display.html/seat.html에서 다른 어떤 상태보다 우선해서 "명령퇴사"로 표시되고 무단외출·자리비움 판정에서 제외됨
    }
...
outings/
  {date}/                       // "YYYY-MM-DD"(로컬 날짜). 하루가 지나도 기록이 남도록 날짜별로 분리(check.html의 "조회 날짜"가 이 키를 고름)
    {studentId}: {
      status: "in" | "out" | "unauthorized" | "away",
      // "out"=정상 외출 체크, "unauthorized"=무단외출, "away"=자리비움 — 뒤 둘은 자동 판단 없이 check.html에서 교사가 직접 표시하는 수동 상태(재실에서만 진입, 재실로만 복귀)
      since: timestamp,
      reason?: string,            // 외출 사유. "외출 체크" 시 프롬프트로 입력(선택, 빈 값 가능). unauthorized/away에는 없음
...
- `students/{grade}`: admin, 또는 `managedGrades`에 해당 학년이 있는 gradeManager, 또는 `managedClasses`에 해당 학년이 있는(반이 하나라도 배정된) teacher만 쓰기 가능 — 셋 다 **학년 단위**까지만 서버에서 강제함
...
- [x] 외출 체크 입력 화면: Realtime DB 연동, 토글 시 `outings/{날짜}/{studentId}` 갱신, 무단외출·자리비움 수동 표시, 지난 날짜 조회·수정
...
- [x] 무단외출·자리비움 수동 상태 + 명령퇴사(기간제 상태) 추가, 현황판·좌석 배치판 표시 반영
