...
- 상단: 등록된 계정 목록 — 아이디·이름·역할 배지(teacher/gradeManager/admin), gradeManager는 담당 학년·담당 실도 함께 표시
  - 본인 계정 행에는 "역할 변경" 버튼이 없음(관리자가 실수로 자기 자신을 강등해 잠기는 것을 방지)
  - "역할 변경" 클릭 시 그 계정 행이 인라인 편집 폼으로 바뀜: 역할 토글(teacher/gradeManager/admin), gradeManager 선택 시 담당 학년(1/2/3학년 토글)·담당 실(rooms 목록에서 토글) 추가 노출 → 저장 시 `users/{uid}` set
