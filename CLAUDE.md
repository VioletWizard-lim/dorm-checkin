...
- 학생 카드 리스트: 아바타, 이름, "학번 · 반", 상태 배지(재실/외출중/자리 없음/명령퇴사), 상태별 조작:
  - **재실** → "외출 체크" 버튼(아래 설명) + "자리 없음으로 표시" 버튼(순회 중 자리에 없는 걸 발견하면 직접 표시)
  - **외출중** → "복귀 체크" 버튼(재실로)
  - **자리 없음** → "재실로 되돌리기" 버튼만 노출(자동 판단 없이 전부 수동 — 순회하는 교사가 직접 표시·해제. 예전엔 "무단외출"/"자리비움" 두 상태로 나눠뒀다가 하나로 합침)
...
- 보기 모드 / 편집 모드 토글
- **보기 모드**에서 학생이 배정된 좌석을 클릭하면(명령퇴사 중인 학생은 클릭 불가) 그 자리에서 출석 상태를 바로 바꿀 수 있음 — check.html까지 갈 필요 없이 순회하며 바로 체크하기 위함:
  - 재실 → "외출"(check.html의 외출 체크와 동일하게 사유·예상 복귀 시각 프롬프트 후 외출증 이메일 발송) / "자리없음" 버튼
  - 외출중 → "복귀" 버튼
  - 자리 없음 → "재실로" 버튼
  - 모든 상태에 "취소" 버튼(적용 안 하고 닫기)
- 편집 모드에서 관리자·학년관리자가 할 수 있는 것(위 출석 상태 변경과는 별개 — 편집 모드에서는 좌석을 눌러도 출석 상태 조작 패널이 뜨지 않음):
...
|---|---|---|---|---|
| 외출 체크 입력/조회 (+ 자리 없음 수동 표시, 지난 기록 수정) — check.html, 좌석 배치판에서도 좌석 클릭으로 가능 | ✅ | ✅ | ✅ | ✅ (check.html만 가능) |
| 현황판·좌석배치판 열람 | ✅ | ✅ | ✅ | ❌ |
...
- [x] 좌석 배치판에서도 좌석을 클릭해 출석 상태(외출/복귀/자리 없음)를 바로 바꿀 수 있게 추가
